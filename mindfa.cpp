...
void get_NEW_start(
	char *newstart,	/* new DFA finals */
	char *oldstart,	/* source DFA finals */
	char stnt[][STATES+1],	/* state name table */
	int n)	/* number of states in 'stnt' */
{
	int i;
	...
	for (i = 0; i < n; i++)
		if (is_subset(oldstart, stnt[i])) *newstart++ = i+'A';
	*newstart++ = '\0';
}
...
int main()
{
	...
	ifstream f;int N;int M;
	f.open("DFA.txt");
	f>>N>>M;int final;int count;int so;int p;int fi;
	N_DFA_states=N;
	N_symbols=M;
	f>>final;
	for(int i=0;i<final;i++)
	{f>>count;s[i]=(char)('A'+count);
	...
	get_NEW_start(NEW_start, DFA_start, StateName, N_optDFA_states);
	...
	cin>>in;
	int str=NEW_start[0]-'A';int curr=str;
	cout<<"Start State : "<<(char)(curr+(int)'A')<<"\n\n";
	for(int h=0;h<in.size();h++)
	{cout<<(char)(str+(int)'A')<<" "<<"on input "<<in[h]<<" goto ";
		curr=OptDFA[curr][in[h]-'a']-'A';
		cout<<(char)(curr+(int)'A')<<"\n";

		str=curr;
	}
cout<<"\nFinal State reached : "<<(char)(curr+(int)'A')<<"\n";
